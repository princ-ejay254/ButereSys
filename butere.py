'''@princ-ejay254 this is my creation
its a small school management system for Shiners academy that handles studentbregistration and evaluation of exams
Its ment to be a standalone system built using python and tkinter framework.
...
connector = sqlite3.connect('Shiners.db')
...
def reset_fields():
   global fname_strvar, sname_strvar, email_strvar, contact_strvar, gender_strvar, dob, stream_strvar
...
main = Tk()
main.title('Butere Boys High School Management System')
...
lf_bg = 'DarkRed'      # Choose a color close to maroon for left_frame
cf_bg = 'AntiqueWhite' # Choose a color close to cream for center_frame
...
Label(main, text="SCHOOL MANAGEMENT SYSTEM", font=headlabelfont, bg='DarkRed').pack(side=TOP, fill=X)
...
right_frame = Frame(main, bg="AntiqueWhite")
...
OptionMenu(left_frame, stream_strvar, 'East', 'West','North','South','Central',"Blue").place(x=40, rely=0.75, relwidth=0.5)
...
Label(right_frame, text='Students Records', font=headlabelfont, bg='DarkRed', fg='LightCyan').pack(side=TOP, fill=X)
